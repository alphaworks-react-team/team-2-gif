	...
	return new Promise(async (resolve, reject) => {
		try {
			const request = await axios.get(
				`https://api.giphy.com/v1/gifs/trending?api_key=${process.env.GIF_KEY}&limit=36`
			);
...
const fetchRandom = () => {
	return new Promise(async (resolve, reject) => {
		try {
			const request = await axios.get(
				`https://api.giphy.com/v1/gifs/random?&api_key=${process.env.GIF_KEY}&limit=40`
			);
			resolve(request.data.data);
		} catch (err) {
			reject(err);
...
const fetchCategories = () => {
	return new Promise(async (resolve, reject) => {
		try {
			const request = await axios.get(
				`https://api.giphy.com/v1/gifs/categories?&api_key=${process.env.GIF_KEY}`
			);
			resolve(request.data.data);
		} catch (err) {
			reject(err);
